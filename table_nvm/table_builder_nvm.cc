...
Status BuildTableNVM(const std::string& dbname, Env* env,
                     ...
                     Iterator* iter, FileMetaData* meta, uint64_t new_seg_num) {
  Status s;
  meta->file_size = 0;
  iter->SeekToFirst();
  ...
  if (iter->Valid()) {
    ...
    if (s.ok()) {
      ...
      SegmentMeta* new_segment =
          new SegmentMeta(0, builder->RawDataSize(), builder->RawDataSize(),
                          builder->MetaDataSize(), meta->number, new_seg_num,
  ...
  if (s.ok() && meta->file_size > 0) {
    Log(options.info_log,
        "Build L0 Table success, file_num:%ld, file size:%ld ", meta->number,
        meta->file_size);
...
TableBuilderNVMSplit::TableBuilderNVMSplit(const Options& option)
    : option_(option), data_offset_(0), meta_offset_(0), closed_(false) {
  buffer_.reserve(option.write_buffer_size);
  // offset_ = 0;
}
...
Status TableBuilderNVMSplit::Finish(WritableFile* file) {
  ...
  for (int i = 0; i < meta_data_.size(); i++) {
    ...
    PutFixed64(&meta_buffer_,
               meta_start_offset + meta_data_[i]->key_offset_meta);
...
uint64_t TableBuilderNVMSplit::FileSize() const {
  return RawDataSize() + MetaDataSize();
}
...
Status TableBuilderNVMSplit::status() const { return Status::OK(); }
}  // namespace leveldb
