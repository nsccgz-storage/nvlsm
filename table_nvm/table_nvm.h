...
private:
   ...
   int find_less_or_equal_idx(const InternalKey &ikey);
    ...
    p<uint> buckets_limit_;
    //only stores the values
    // data format
    // [val size] [val]
    // pmem::obj::vector<pmem::obj::string> buckets_rep_;
    pmem::obj::string bucket_rep;

    p<uint64_t> entry_num_;
    pmem::obj::vector<KeysMetadata> buckets_keys;
